	...
	function b(a) {
		...
		function c(b) {
			if (!d.locked) {
				...
				var c = a.offset();
				d.x = Math.max(0, Math.min(b.pageX -
					c.left, a.width())), d.y = Math.max(0, Math.min(b.pageY -
					c.top, a.height())), a.triggerRedrawOverlay()
			}
		}
		...
		a.setCrosshair = function (b) {
			...
			if (c.mode) {
				...
				if (b.save(), b.translate(e.left, e.top), -1 != d.x) {
					...
					if (b.strokeStyle = c.color, b.lineWidth = c.lineWidth, b.lineJoin = "round", b.beginPath(), -1 !=
						c.mode.indexOf("x")) {
