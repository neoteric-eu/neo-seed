...
!function (a) {
	...
	function h(c, d) {
		...
		mb = null != cb.neoDatepicker ? ab.moment(cb.neoDatepicker) : ab.getNow(), cb.droppable &&
	...
	function j(b) {
		...
		function h(b) {
			...
			if (a.isFunction(b) || a.isArray(b) ? c = {"events": b} : "string" ==
				...
				(c = a.extend({}, b), "string" == typeof c.className &&
	...
	function $(b, c, d) {
		...
		function h() {
			var c = i();
			J &&
			J.remove(), J = a(c).appendTo(b), K = J.find("thead"), L = K.find(".fc-day-header"), M = J.find("tbody"), N = M.find("tr"), O = M.find(".fc-day"), P = N.find("td:first-child"), Q = N.eq(0).find(".fc-day > div"), R = N.eq(0).find(".fc-day-content > div"), H(K.add(K.find("tr"))), H(N), N.eq(0).addClass("fc-first"), N.filter(":last").addClass("fc-last"), O.each(function (b,
				c) {
	...
	function eb(c, d, e) {
		...
		function n(b) {
			...
			db &&
			(c = c.add(db.find("th:first"))), c = c.add(jb.find("th:first")), nb = 0, q(c.width("").each(function (b,
				c) {
			...
			G(d), d.mousedown(function (a) {
				...
				if (1 == f.which) {
					...
					V(), a("body").css("cursor", g +
						"-resize").one("mouseup", h), y("eventResizeStart", d[0], b, f, {}), m.start(function (d,
						f) {
						if (d) {
							var h = Z(f), i = Z(d);
