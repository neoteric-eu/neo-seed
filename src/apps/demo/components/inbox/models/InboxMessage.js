define(['apps/demo/components/inbox/module', 'lodash'], function (module, _) {

	"use strict";

	return module.registerFactory('InboxMessage', function ($resource) {
		var InboxMessage = $resource('apps/demo/api/messages.json/:id', {'id': '@_id'}, {
			get: {
				url: 'app-demo/api/message.json',
				isArray: false
			}
		});
		...
		_.extend(InboxMessage.prototype, {
			...
			getBodyTeaser: function () {
				...
				return clearBody.length > teaserMaxLength ? clearBody.substring(0, teaserMaxLength) +
				'...' : clearBody;
			}
		});
