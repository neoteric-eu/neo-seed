...
!function (a) {
	...
	function f(b) {
		function c(c, d) {
			a.isPlainObject(d) && a.isPlainObject(b[c]) && !g(c) ? b[c] = f({}, b[c], d) : void 0 !==
			d &&
			(b[c] = d)
		}
	...
	function h(c, d) {
		...
		function g() {
			ib = cb.theme ? "ui" : "fc", c.addClass("fc"), c.addClass(cb.isRTL ? "fc-rtl" : "fc-ltr"), cb.theme &&
			c.addClass("ui-widget"), hb = a("<div class='fc-content' />").prependTo(c), fb = new i(ab, cb), gb = fb.render(), gb &&
			c.prepend(gb), o(cb.defaultView), cb.handleWindowResize && a(window).resize(v), n() || h()
		}
		...
		function k() {
			jb &&
			(Z("viewDestroy", jb, jb, jb.element), jb.triggerEventDestroy()), a(window).unbind("resize", v), cb.droppable &&
			a(document).off("dragstart", $).off("dragstop", _), jb.selectionManagerDestroy &&
			jb.selectionManagerDestroy(), fb.destroy(), hb.remove(), c.removeClass("fc fc-ltr fc-rtl ui-widget")
		}
		...
		function p(b) {
			sb++, jb &&
			(Z("viewDestroy", jb, jb, jb.element), J(), jb.triggerEventDestroy(), U(), jb.element.remove(), fb.deactivateButton(jb.name)), fb.activateButton(b), jb = new Fb[b](a("<div class='fc-view fc-view-" +
			b +
			"' />").appendTo(hb), ab), q(), V(), sb--
		}
		...
		function r(a) {
			sb++, jb.start && (Z("viewDestroy", jb, jb, jb.element), J(), B()), U(), a &&
			(mb = jb.incrementDate(mb, a)), jb.render(mb.clone()), u(), V(), (jb.afterRender ||
			A)(), G(), H(), Z("viewRender", jb, jb, jb.element), sb--, C()
		}
		...
		function t() {
			lb = cb.contentHeight ? cb.contentHeight : cb.height ? cb.height -
			(gb ? gb.height() : 0) -
			w(hb) : Math.round(hb.width() / Math.max(cb.aspectRatio, .5))
		}

		function u() {
			void 0 ===
			lb &&
			t(), sb++, jb.setHeight(lb), jb.setWidth(hb.width()), sb--, kb = c.outerWidth()
		}
		...
		function v(a) {
			if (!sb && a.target === window)if (jb.start) {
				...
				setTimeout(function () {
					...
					kb !=
					(kb = c.outerWidth()) &&
					(sb++, s(), jb.trigger("windowResize", qb), sb--)
		...
		function Y(a, b) {
			return void 0 === b ? cb[a] : void(("height" ==
			a ||
			"contentHeight" ==
			a ||
			"aspectRatio" ==
		...
		d = d || {};
		var bb, cb = f({}, Bb, d);
		bb = cb.lang in Cb ? Cb[cb.lang] : Cb[Bb.lang], bb && (cb = f({}, Bb, bb, d)), cb.isRTL &&
		(cb = f({}, Bb, Db, bb ||
		{}, d)), ab.options = cb, ab.render = e, ab.destroy = k, ab.refetchEvents = x, ab.reportEvents = E, ab.reportEventChange = F, ab.rerenderEvents = y, ab.changeView = o, ab.select = I, ab.unselect = J, ab.prev = K, ab.next = L, ab.prevYear = M, ab.nextYear = N, ab.today = O, ab.gotoDate = Q, ab.incrementDate = R, ab.getDate = T, ab.getCalendar = W, ab.getView = X, ab.option = Y, ab.trigger = Z;
		var db = l(b.langData(cb.lang));
		if (cb.monthNames && (db._months = cb.monthNames), cb.monthNamesShort &&
			(db._monthsShort = cb.monthNamesShort), cb.dayNames &&
			(db._weekdays = cb.dayNames), cb.dayNamesShort &&
			(db._weekdaysShort = cb.dayNamesShort), null != cb.firstDay) {
			var eb = l(db._week);
		...
		ab.defaultAllDayEventDuration = b.duration(cb.defaultAllDayEventDuration), ab.defaultTimedEventDuration = b.duration(cb.defaultTimedEventDuration), ab.moment = function () {
			var a;
			return "local" === cb.timezone ? (a = Eb.moment.apply(null, arguments), a.hasTime() &&
			a.local()) : a = "UTC" ===
			cb.timezone ? Eb.moment.utc.apply(null, arguments) : Eb.moment.parseZone.apply(null, arguments), a._lang = db, a
			...
			return "function" == typeof b ? b(a) : "local" === b ? a.week() : "ISO" ===
			b.toUpperCase() ? a.isoWeek() : void 0
			...
			var c = b.clone();
			return a ? c.stripTime().add(ab.defaultAllDayEventDuration) : c.add(ab.defaultTimedEventDuration), ab.getIsAmbigTimezone() &&
			c.stripZone(), c
		...
		mb = null != cb.defaultDate ? ab.moment(cb.defaultDate) : ab.getNow(), cb.droppable &&
		a(document).on("dragstart", $).on("dragstop", _)
	}
		...
		function f(d) {
			...
			return f && a.each(f.split(" "), function (d) {
				...
				a.each(this.split(","), function (d, g) {
					if ("title" == g)e.append("<span class='fc-header-title'><h2>&nbsp;</h2></span>"), f &&
					f.addClass(m + "-corner-right"), f = null; else {
						var h;
						if (b[g] ? h = b[g] : Fb[g] && (h = function () {
							...
							i = n ? E(n) : j && c.theme ? "<span class='ui-icon ui-icon-" + j + "'></span>" : k &&
							!c.theme ? "<span class='fc-icon fc-icon-" + k + "'></span>" : E(l || g);
							var o = a("<span class='fc-button fc-button-" +
							g +
							" " +
							m +
							"-state-default'>" +
							...
							"</span>").click(function () {
								...
								o.not("." + m + "-state-active").not("." + m + "-state-disabled").addClass(m +
								"-state-down")
							}).mouseup(function () {
	...
	function j(b) {
		...
		function f(c, d) {
			...
			if (i)a.isFunction(i) ? (s(), i.call(x, y.clone(), z.clone(), b.timezone, function (a) {
				...
				if (j) {
					...
					var o  = a.extend({}, k ||
					{}), p = K(c.startParam, b.startParam), q = K(c.endParam, b.endParam), r = K(c.timezoneParam, b.timezoneParam);
					p && (o[p] = y.format()), q && (o[q] = z.format()), b.timezone &&
					"local" !=
					b.timezone &&
					(o[r] = b.timezone), s(), a.ajax(a.extend({}, Gb, c, {
						"data": o,
						"success": function (b) {
							b = b || [];
		...
		function h(b) {
			...
			if (a.isFunction(b) || a.isArray(b) ? c = {"events": b} : "string" ==
				typeof b ? c = {"url": b} : "object" ==
				typeof b &&
				(c = a.extend({}, b), "string" ==
				typeof c.className &&
				(c.className = c.className.split(/\s+/))), c) {
		...
		function o(a) {
			var b, c, d, e;
			for (b = 0; b < L.length; b++)if (c = L[b], c._id == a._id && c !== a)for (d = 0; d <
			M.length; d++)e = M[d], void 0 !== a[e] && (c[e] = a[e])
		}
		...
		function q(b) {
			...
			for (null == b ? b = function () {
				...
				return a._id == c
			}), L = a.grep(L, b, !0), d = 0; d < F.length; d++)a.isArray(F[d].events) &&
			(F[d].events = a.grep(F[d].events, b, !0));
			C(L)
		}
		...
		function u(c, d) {
			var e, f, g, h, i = {};
			return b.eventDataTransform && (c = b.eventDataTransform(c)), d &&
			d.eventDataTransform &&
			(c = d.eventDataTransform(c)), e = x.moment(c.start || c.date), e.isValid() &&
			(f = null, !c.end || (f = x.moment(c.end), f.isValid())) ? (g = c.allDay, void 0 ===
			g &&
			(h = K(d ? d.allDayDefault : void 0, b.allDayDefault), g = void 0 !== h ? h : !(e.hasTime() ||
			f &&
			f.hasTime())), g ? (e.hasTime() && e.stripTime(), f &&
			f.hasTime() &&
			f.stripTime()) : (e.hasTime() || (e = x.rezoneDate(e)), f &&
			!f.hasTime() &&
			(f = x.rezoneDate(f))), a.extend(i, c), d && (i.source = d), i._id = c._id ||
			(void 0 === c.id ? "_fc" + Hb++ : c.id + ""), i.className = c.className ? "string" ==
			typeof c.className ? c.className.split(/\s+/) : c.className : [], i.allDay = g, i.start = e, i.end = f, b.forceEventDuration &&
			!i.end &&
			(i.end = D(i)), k(i), i) : void 0
		}

		function v(a, b, c) {
			var d, e, f, g, h = a._allDay, i = a._start, j = a._end, k = !1;
			return b || c || (b = a.start, c = a.end), d = a.allDay != h ? a.allDay : !(b ||
			c).hasTime(), d && (b && (b = b.clone().stripTime()), c && (c = c.clone().stripTime())), b &&
			(e = d ? n(b, i.clone().stripTime()) : n(b, i)), d != h ? k = !0 : c &&
			(f = n(c || x.getDefaultEventEnd(d, b || i), b || i).subtract(n(j ||
			x.getDefaultEventEnd(h, i), i))), g = w(r(a._id), k, d, e, f), {
				"dateDelta": e,
				"durationDelta": f,
				"undo": g
			}
		...
		function w(c, d, e, f, g) {
			...
			return a.each(c, function (a, c) {
				var j = c._allDay, l = c._start, m = c._end, n = null != e ? e : j, o = l.clone(), p = !d &&
				m ? m.clone() : null;
				n ? (o.stripTime(), p && p.stripTime()) : (o.hasTime() || (o = x.rezoneDate(o)), p &&
				!p.hasTime() &&
				(p = x.rezoneDate(p))), p ||
				!b.forceEventDuration &&
				!+g ||
				(p = x.getDefaultEventEnd(n, o)), o.add(f), p && p.add(f).add(g), h &&
				(+f || +g) &&
				(o.stripZone(), p &&
				p.stripZone()), c.allDay = n, c.start = o, c.end = p, k(c), i.push(function () {
	...
	function p(b, c, d) {
		b.unbind("mouseover").mouseover(function (b) {
			...
			void 0 !==
			(f = e._fci) &&
			(e._fci = void 0, g = c[f], d(g.event, g.element, g), a(b.target).trigger(b)), b.stopPropagation()
		})
	}
	...
	function t(b) {
		return (parseFloat(a.css(b[0], "paddingLeft", !0)) || 0) +
			(parseFloat(a.css(b[0], "paddingRight", !0)) || 0)
	}

	function u(b) {
		return (parseFloat(a.css(b[0], "marginLeft", !0)) || 0) +
			(parseFloat(a.css(b[0], "marginRight", !0)) || 0)
	}

	function v(b) {
		return (parseFloat(a.css(b[0], "borderLeftWidth", !0)) || 0) +
	...
	function x(b) {
		return (parseFloat(a.css(b[0], "paddingTop", !0)) || 0) +
			(parseFloat(a.css(b[0], "paddingBottom", !0)) || 0)
	}

	function y(b) {
		return (parseFloat(a.css(b[0], "marginTop", !0)) || 0) +
			(parseFloat(a.css(b[0], "marginBottom", !0)) || 0)
	}

	function z(b) {
		return (parseFloat(a.css(b[0], "borderTopWidth", !0)) || 0) +
	...
	function D(a, b) {
		...
		for (var c, d = b.split(/(?=[A-Z])/), e = d.length - 1; e >=
		0; e--)if (c = a[d[e].toLowerCase()], void 0 !== c)return c;
		return a["default"]
	}
	...
	function I(a, b) {
		var c  = a.source || {}, d = a.color, e = c.color, f = b("eventColor"), g = a.backgroundColor ||
			d ||
			c.backgroundColor ||
			e ||
			...
			e ||
			b("eventBorderColor") ||
			f, i = a.textColor || c.textColor || b("eventTextColor"), j = [];
		return g && j.push("background-color:" + g), h && j.push("border-color:" + h), i &&
		j.push("color:" + i), j.join(";")
	}
	...
	function L(c, d, e) {
		...
		return b.isMoment(j) ? (i = b.apply(null, c), j._ambigTime &&
		(i._ambigTime = !0), j._ambigZone && (i._ambigZone = !0)) : o(j) ||
		void 0 ===
		j ? i = b.apply(null, c) : (f = !1, g = !1, k ? Jb.test(j) ? (j += "-01", c = [j], f = !0, g = !0) : (h = Kb.exec(j)) &&
		(f = !h[5], g = !0) : a.isArray(j) &&
		(g = !0), i = d ? b.utc.apply(b, c) : b.apply(null, c), f ? (i._ambigTime = !0, i._ambigZone = !0) : e &&
		(g ? i._ambigZone = !0 : k && i.zone(j))), new M(i)
	}
	...
	function N(a) {
		var b, c = [], d = !1, e = !1;
		for (b = 0; b < a.length; b++)c.push(Eb.moment(a[b])), d = d || c[b]._ambigTime, e = e ||
		c[b]._ambigZone;
	...
	function R(a, b) {
		var c, d;
		return "string" == typeof b ? b : (c = b.token) ? Lb[c] ? Lb[c](a) : O(a, c) : b.maybe &&
		(d = Q(a, b.maybe), d.match(/[1-9]/)) ? d : ""
	}

	function S(a, b, c, d, e) {
		return a = Eb.moment.parseZone(a), b = Eb.moment.parseZone(b), c = a.lang().longDateFormat(c) ||
		c, d = d || " - ", T(a, b, V(c), d, e)
	}
	...
	function U(a, b, c) {
		var d, e;
		return "string" == typeof c ? c : (d = c.token) &&
		(e = Mb[d.charAt(0)], e && a.isSame(b, e)) ? O(a, d) : !1
	}
	...
	function W(a) {
		for (var b, c = [], d = /\[([^\]]*)\]|\(([^\)]*)\)|(LT|(\w)\4*o?)|([^\w\[\(]+)/g; b = d.exec(a);)b[1] ? c.push(b[1]) : b[2] ? c.push({"maybe": W(b[2])}) : b[3] ? c.push({"token": b[3]}) : b[5] &&
		c.push(b[5]);
		return c
	}
	...
	function X(a, b) {
		...
		function d(a) {
			e.intervalStart = a.clone().stripTime().startOf("month"), e.intervalEnd = e.intervalStart.clone().add("months", 1), e.start = e.intervalStart.clone(), e.start = e.skipHiddenDays(e.start), e.start.startOf("week"), e.start = e.skipHiddenDays(e.start), e.end = e.intervalEnd.clone(), e.end = e.skipHiddenDays(e.end, -1, !0), e.end.add("days", (7 -
			e.end.weekday()) % 7), e.end = e.skipHiddenDays(e.end, -1, !0);
			var c = Math.ceil(e.end.diff(e.start, "weeks", !0));
			"fixed" ==
			e.opt("weekMode") &&
			(e.end.add("weeks", 6 -
			c), c = 6), e.title = b.formatDate(e.intervalStart, e.opt("titleFormat")), e.renderBasic(c, e.getCellsPerWeek(), !0)
		}
	...
	function $(b, c, d) {
		...
		function h() {
			var c = i();
			J &&
			J.remove(), J = a(c).appendTo(b), K = J.find("thead"), L = K.find(".fc-day-header"), M = J.find("tbody"), N = M.find("tr"), O = M.find(".fc-day"), P = N.find("td:first-child"), Q = N.eq(0).find(".fc-day > div"), R = N.eq(0).find(".fc-day-content > div"), H(K.add(K.find("tr"))), H(N), N.eq(0).addClass("fc-first"), N.filter(":last").addClass("fc-last"), O.each(function (b, c) {
		...
		function i() {
			var a = "<table class='fc-border-separate' style='width:100%' cellspacing='0'>" +
				j() +
				k() +
				"</table>";
			return a
		...
		function j() {
			...
			for (d += "<thead><tr>", fb &&
			(d += "<th class='fc-week-number " +
			c +
			"'>" +
			E(gb("weekNumberTitle")) +
			"</th>"), a = 0; Y > a; a++)b = lb(0, a), d += "<th class='fc-day-header fc-" +
			Ib[b.day()] +
			" " +
			c +
			"'>" +
		...
		function k() {
			...
			for (e += "<tbody>", a = 0; X > a; a++) {
				for (e += "<tr class='fc-week'>", fb &&
				(c = lb(a, 0), e += "<td class='fc-week-number " +
				d +
				"'><div>" +
				E(pb(c)) +
				"</div></td>"), b = 0; Y > b; b++)c = lb(a, b), e += l(c);
				e += "</tr>"
			}
			return e += "</tbody>"
		}
		...
		function l(a) {
			var b                  = I.intervalStart.month(), d = c.getNow().stripTime(), e = "", f = db +
				"-widget-content", g = ["fc-day", "fc-" + Ib[a.day()], f];
			return a.month() !=
			b &&
			g.push("fc-other-month"), a.isSame(d, "day") ? g.push("fc-today", db +
			"-state-highlight") : g.push(d > a ? "fc-past" : "fc-future"), e += "<td class='" +
			g.join(" ") +
			"' data-date='" +
			a.format() +
			"'><div>", Z &&
			(e += "<div class='fc-day-number'>" +
			a.date() +
		...
		function m(b) {
			...
			"variable" == gb("weekMode") ? c = d = Math.floor(f / (1 == X ? 2 : 6)) : (c = Math.floor(f /
			X), d = f - c * (X - 1)), P.each(function (b, f) {
				X > b && (e = a(f), e.find("> div").css("min-height", (b == X - 1 ? d : c) - w(e)))
			})
		}

		function n(a) {
			T = a, bb.clear(), cb.clear(), W = 0, fb &&
			(W = K.find("th.fc-week-number").outerWidth()), V = Math.floor((T - W) /
			Y), q(L.slice(0, -1), V)
		}
	...
	function eb(c, d, e) {
		...
		function h() {
			var d, e, f, g, h = Hb + "-widget-header", j = Hb + "-widget-content", k = rb.asMinutes() %
				15 ===
				0;
			for (i(), bb = a("<div style='position:absolute;z-index:2;left:0;width:100%'/>").appendTo(c), Nb("allDaySlot") ? (cb = a("<div class='fc-event-container' style='position:absolute;z-index:8;top:0;left:0'/>").appendTo(bb), d = "<table style='width:100%' class='fc-agenda-allday' cellspacing='0'><tr><th class='" +
			h +
			" fc-agenda-axis'>" +
			(Nb("allDayHTML") || E(Nb("allDayText"))) +
			"</th><td><div class='fc-day-content'><div style='position:relative'/></div></td><th class='" +
			h +
			" fc-agenda-gutter'>&nbsp;</th></tr></table>", db = a(d).appendTo(bb), eb = db.find("tr"), s(eb.find("td")), bb.append("<div class='fc-agenda-divider " +
			h +
			"'><div class='fc-agenda-divider-inner'/></div>")) : cb = a([]), gb = a("<div style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto'/>").appendTo(bb), hb = a("<div style='position:relative;width:100%;overflow:hidden'/>").appendTo(gb), ib = a("<div class='fc-event-container' style='position:absolute;z-index:8;top:0;left:0'/>").appendTo(hb), d = "<table class='fc-agenda-slots' style='width:100%' cellspacing='0'><tbody>", e = b.duration(+Kb), Cb = 0; Lb >
			     e;)f = T.start.clone().time(e), g = f.minutes(), d += "<tr class='fc-slot" +
			Cb +
			" " +
			(g ? "fc-minor" : "") +
			...
			"</th><td class='" +
			j +
			"'><div style='position:relative'>&nbsp;</div></td></tr>", e.add(rb), Cb++;
		...
		function i() {
			var b = j();
			U &&
			U.remove(), U = a(b).appendTo(c), V = U.find("thead"), W = V.find("th").slice(1, -1), X = U.find("tbody"), Y = X.find("td").slice(0, -1), Z = Y.find("> div"), $ = Y.find(".fc-day-content > div"), _ = Y.eq(0), ab = Z.eq(0), H(V.add(V.find("tr"))), H(X.add(X.find("tr")))
		}

		function j() {
			var a = "<table style='width:100%' class='fc-agenda-days fc-border-separate' cellspacing='0'>" +
				k() +
				l() +
				"</table>";
			return a
		...
		function k() {
			...
			for (e += "<thead><tr>", Nb("weekNumbers") ? (a = Vb(0, 0), b = Zb(a), Jb ? b += Nb("weekNumberTitle") : b = Nb("weekNumberTitle") +
			b, e += "<th class='fc-agenda-axis fc-week-number " +
			d +
			"'>" +
			E(b) +
			"</th>") : e += "<th class='fc-agenda-axis " + d + "'>&nbsp;</th>", c = 0; Bb >
			     c; c++)a = Vb(0, c), e += "<th class='fc-" +
			Ib[a.day()] +
			" fc-col" +
			c +
			" " +
		...
		function l() {
			var a, b, c, e, f, g   = Hb + "-widget-header", h = Hb +
				"-widget-content", i = d.getNow().stripTime(), j = "";
			for (j += "<tbody><tr><th class='fc-agenda-axis " + g + "'>&nbsp;</th>", c = "", b = 0; Bb >
			b; b++)a = Vb(0, b), f = ["fc-col" + b, "fc-" +
			Ib[a.day()], h], a.isSame(i, "day") ? f.push(Hb + "-state-highlight", "fc-today") : f.push(i >
			a ? "fc-past" : "fc-future"), e = "<td class='" +
			f.join(" ") +
			"'><div><div class='fc-day-content'><div style='position:relative'>&nbsp;</div></div></div></td>", c += e;
		...
		function n(b) {
			...
			db &&
			(c = c.add(db.find("th:first"))), c = c.add(jb.find("th:first")), nb = 0, q(c.width("").each(function (b, c) {
			...
			pb = gb.width() -
			e, pb ? (q(d, pb), d.show().prev().removeClass("fc-last")) : d.hide().prev().addClass("fc-last"), ob = Math.floor((e -
			nb) / Bb), q(W.slice(0, -1), ob)
		}
		...
		function u(a) {
			if (!Nb("selectable")) {
				var b   = Math.min(Bb - 1, Math.floor((a.pageX - U.offset().left - nb) /
				ob)), c = Vb(0, b), e = this.parentNode.className.match(/fc-slot(\d+)/);
				if (e) {
		...
		function I(a) {
			...
			return Nb("allDaySlot") && e--, e >=
			0 &&
			(c.time(b.duration(Kb + e * tb)), c = d.rezoneDate(c)), c
		}
		...
		G(c.addClass("fc-agenda")), Db = new xb(function (b, c) {
			...
			}), g[1] = f + e.outerWidth(), Nb("allDaySlot") &&
			(e = eb, f = e.offset().top, b[0] = [f, f + e.outerHeight()]);
			for (var h = hb.offset().top, i = gb.offset().top, j = i + gb.outerHeight(), k = 0; Cb *
			ub >
			k; k++)b.push([d(h + zb * k), d(h + zb * (k + 1))])
	...
	function fb() {
		...
		function e(a) {
			...
			for (c = 0; h >
			c; c++)for (b = F(0, c), g = f(a, b.clone().time(i), b.clone().time(j)), g = gb(g), d = 0; d <
			g.length; d++)e = g[d], e.col = c, k.push(e);
			return k
		}
		...
		function f(a, b, c) {
			...
			for (d = 0; m > d; d++)e = a[d], f = e.start.clone().stripZone(), g = $(e).stripZone(), g >
			b &&
			c >
			f &&
			(b > f ? (h = b.clone(), j = !1) : (h = f, j = !0), g >
			c ? (i = c.clone(), k = !1) : (i = g, k = !0), l.push({
				"event": e,
				"start": h,
				"end": i,
				"isStart": j,
		...
		function g(b, c) {
			...
			for (d = 0; E >
			d; d++)e = b[d], f = e.event, g = A(e.start, e.start), j = A(e.end, e.start), k = C(e.col), l = D(e.col), m = l -
			k, l -= .025 * m, m = l - k, q = m *
			(e.forwardCoord - e.backwardCoord), n("slotEventOverlap") &&
			(q = Math.max(2 * (q - 10), q)), H ? (t = l - e.backwardCoord * m, r = t - q) : (r = k +
			e.backwardCoord *
			m, t = r + q), r = Math.max(r, k), t = Math.min(t, l), q = t -
			r, e.top = g, e.left = r, e.outerWidth = q, e.outerHeight = j - g, F += h(f, e);
			for (G[0].innerHTML = F, u = G.children(), d = 0; E >
			d; d++)e = b[d], f = e.event, v = a(u[d]), x = o("eventRender", f, f, v), x ===
			!1 ? v.remove() : (x && x !== !0 && (v.remove(), v = a(x).css({
				"position": "absolute",
				"top": e.top,
				"left": e.left
			}).appendTo(G)), e.element = v, f._id === c ? i(f, v, e) : v[0]._fci = d, O(f, v));
			for (p(G, b, i), d = 0; E > d; d++)e = b[d], (v = e.element) &&
			(e.vsides = w(v, !0), e.hsides = s(v, !0), z = v.find(".fc-event-title"), z.length &&
			(e.contentTop = z[0].offsetTop));
			for (d = 0; E > d; d++)e = b[d], (v = e.element) &&
			(v[0].style.width = Math.max(0, e.outerWidth - e.hsides) +
			"px", B = Math.max(0, e.outerHeight - e.vsides), v[0].style.height = B +
			"px", f = e.event, void 0 !==
			e.contentTop &&
			B -
			e.contentTop <
			10 &&
			(v.find("div.fc-event-time").text(Z(f.start, n("timeFormat")) +
			" - " +
			f.title), v.find("div.fc-event-title").remove()), o("eventAfterRender", f, f, v))
		}

		function h(a, b) {
			var c = "<", d = a.url, e = I(a, n), f = ["fc-event", "fc-event-vert"];
			return q(a) && f.push("fc-event-draggable"), b.isStart && f.push("fc-event-start"), b.isEnd &&
			f.push("fc-event-end"), f = f.concat(a.className), a.source &&
			(f = f.concat(a.source.className || [])), c += d ? "a href='" +
			E(a.url) +
			"'" : "div", c += " class='" +
			f.join(" ") +
			...
			E(a.title || "") +
			"</div></div><div class='fc-event-bg'></div>", b.isEnd &&
			t(a) &&
			(c += "<div class='ui-resizable-handle ui-resizable-s'>=</div>"), c += "</" +
			(d ? "a" : "div") +
			">"
		}
			...
			c.draggable({
				...
				"start": function (b, d) {
					o("eventDragStart", c[0], a, b, d), Q(a, c), f = c.width(), k.start(function (b, d) {
						if (U(), b) {
							...
							h = k.diff(f, "days"), b.row ? i ? j &&
							(c.width(l - 10), r(c, Y.defaultTimedEventDuration /
							p *
							q), c.draggable("option", "grid", [l, 1]), j = !1) : g = !0 : (T(a.start.clone().add("days", h), $(a).add("days", h)), e()), g = g ||
							j &&
							!h
						} else e(), g = !0;
				...
				"stop": function (d, f) {
					if (k.stop(), U(), o("eventDragStop", c[0], a, d, f), g)e(), c.css("filter", ""), P(a, c); else {
						...
						j ||
						(l = Math.round((c.offset().top - N().offset().top) / t), i = b.duration(m +
						l *
						s), n = Y.rezoneDate(n.clone().time(i))), R(c[0], a, n, d, f)
					}
				}
			})
		}
		...
		function k(a, b, c) {
			function d() {
				U(), h &&
				(j ? (c.hide(), b.draggable("option", "grid", null), T(t, u)) : (e(), c.css("display", ""), b.draggable("option", "grid", [x, y])))
			}
			...
			b.draggable({
				...
				"drag": function (c, e) {
					...
					(h != i || j != k || l != p || r != s) &&
					(j ? (t = a.start.clone().stripTime().add("days", q), u = t.clone().add(Y.defaultAllDayEventDuration)) : (t = a.start.clone().add(r *
					z).add("days", q), u = $(a).add(r *
					z).add("days", q)), d(), i = h, k = j, p = l, s = r), b.draggable("option", "revert", !h)
				},
				"stop": function (c, e) {
					U(), o("eventDragStop", b[0], a, c, e), h &&
					(j ||
					q ||
					r) ? R(b[0], a, t, c, e) : (h = !0, j = !1, l = 0, q = 0, r = 0, d(), b.css("filter", ""), b.css(f), P(a, b))
				}
			})
		}
	...
	function ib(a) {
		...
		a.length; f++)mb(e, a[f], e.forwardSegs)
	}
	...
	function kb(a, b, c) {
		...
		if (void 0 === a.forwardCoord)for (e.length ? (e.sort(ob), kb(e[0], b +
		1, c), a.forwardCoord = e[0].backwardCoord) : a.forwardCoord = 1, a.backwardCoord = a.forwardCoord -
		(a.forwardCoord - c) /
		(b + 1), d = 0; d < e.length; d++)kb(e[d], 0, a.forwardCoord)
	}
	...
	function qb(c, d, e) {
		...
		function n(a, b) {
			b.click(function (c) {
				...
				b.hasClass("ui-resizable-resizing") ? void 0 : h("eventClick", this, a, c)
			}).hover(function (b) {
		...
		function E(a) {
			var b = G.getColCnt(), c = S ? -1 : 1, d = S ? b - 1 : 0, e = Math.floor(a / b), f = (a %
				b +
				b) % b * c + d;
			return {"row": e, "col": f}
		}
		...
		G.getEventTimeText = function (a) {
			var b, c;
			return 2 ===
			arguments.length ? (b = arguments[0], c = arguments[1]) : (b = a.start, c = a.end), c &&
			f("displayEventEnd") ? d.formatRange(b, c, f("timeFormat")) : d.formatDate(b, f("timeFormat"))
		...
		!function () {
			...
			for (var b = 0, c = 0; 7 > b; b++)Q[b] = c, P[b] = -1 != a.inArray(b, O), P[b] ||
			(R[c] = b, c++);
	...
	function rb() {
		...
		function d(b, c, d) {
			...
			return g(n), f = h(n), m[0].innerHTML = f, i = m.children(), c &&
			l.append(i), j(n, i), tb(n, function (a, b) {
				a.hsides = s(b, !0)
			}), tb(n, function (a, b) {
		...
		function i(a) {
			...
			z(d) && f.push("fc-event-draggable"), a.isStart && f.push("fc-event-start"), a.isEnd &&
			f.push("fc-event-end"), f = f.concat(d.className), d.source &&
			(f = f.concat(d.source.className || []));
			var g = I(d, x);
			return b += e ? "<a href='" + E(e) + "'" : "<div", b += " class='" +
			f.join(" ") +
			"' style='position:absolute;left:" +
			a.left +
			"px;" +
			g +
			"'><div class='fc-event-inner'>", !d.allDay &&
			a.isStart &&
			(b += "<span class='fc-event-time'>" +
			E(w.getEventTimeText(d)) +
			"</span>"), b += "<span class='fc-event-title'>" +
			E(d.title || "") +
			"</span></div>", d.allDay &&
			a.isEnd &&
			A(d) &&
			(b += "<div class='ui-resizable-handle ui-resizable-" +
			(c ? "w" : "e") +
			"'>&nbsp;&nbsp;&nbsp;</div>"), b += "</" + (e ? "a" : "div") + ">"
		}
		...
		function l(a) {
			for (var b, c = L(), d = M(), e = [], f = m(a), g = 0; c > g; g++) {
				...
				for (var j = 0; j < h.length; j++) {
					var k = h[j];
					for (k.top = C(i.slice(k.leftCol, k.rightCol + 1)), b = k.leftCol; b <=
					k.rightCol; b++)i[b] = k.top + k.outerHeight
				}
				e.push(C(i))
			}
			return e
		...
		function m(a) {
			var b, c, d, e = L(), f = [];
			for (b = 0; b < a.length; b++)c = a[b], d = c.row, c.element &&
			(f[d] ? f[d].push(c) : f[d] = [c]);
		...
		function t(a, b, c) {
			z(a) && w.draggableDayEvent(a, b, c), a.allDay &&
			c.isEnd &&
			A(a) &&
			w.resizableDayEvent(a, b, c), D(a, b)
		}
		...
		function v(b, d, e) {
			...
			G(d), d.mousedown(function (a) {
				...
				function h(c) {
					y("eventResizeStop", d[0], b, c, {}), a("body").css("cursor", ""), m.stop(), U(), j &&
					K(d[0], b, k, c, {}), setTimeout(function () {
						i = !1
					}, 0)
				}
				...
				if (1 == f.which) {
					...
					V(), a("body").css("cursor", g +
					"-resize").one("mouseup", h), y("eventResizeStart", d[0], b, f, {}), m.start(function (d, f) {
						if (d) {
							var h = Z(f), i = Z(d);
							if (i = Math.max(i, p), j = $(i) - $(h), k = cb(b).add("days", j), j) {
								o.end = k;
								var m = l;
								l = c(o, e.row, n), l = a(l), l.find("*").css("cursor", g + "-resize"), m &&
								m.remove(), H(b)
		...
		function f(b) {
			...
			if (1 == b.which && j("selectable")) {
				...
				g.start(function (a, b) {
					n(), a &&
					f(a) ? (k = [c(b), c(a)].sort(B), m(k[0], k[1].clone().add("days", 1))) : k = null
				}, b), a(document).one("mouseup", function (a) {
	...
	function wb() {
		function b(b, c) {
			var d = f.shift();
			return d ||
			(d = a("<div class='fc-cell-overlay' style='position:absolute;z-index:3'/>")), d[0].parentNode !=
			c[0] &&
			d.appendTo(c), e.push(d.css(b).show()), d
		}
	...
	function yb(b) {
		function c(a) {
			...
			(Boolean(c) !== Boolean(g) || c && (c.row != g.row || c.col != g.col)) &&
			(c ? (f || (f = c), e(c, f, c.row - f.row, c.col - f.col)) : e(c, f), g = c)
		}
	...
	a.fn.fullCalendar = function (b) {
		...
		return this.each(function (e, f) {
			...
			"string" == typeof b ? j &&
			a.isFunction(j[b]) &&
			(g = j[b].apply(j, c), e || (d = g), "destroy" === b && i.removeData("fullCalendar")) : j ||
			(j = new h(i, b), i.data("fullCalendar", j), j.render())
		}), d
		...
		e || (e = Cb[b] = {}), f(e, {
			...
			"titleFormat": {
				"month": d.showMonthAfterYear ? "YYYY[" +
				d.yearSuffix +
				"] MMMM" : "MMMM YYYY[" + d.yearSuffix + "]"
			},
			"defaultButtonText": {"prev": F(d.prevText), "next": F(d.nextText), "today": F(d.currentText)}
		}), a.datepicker &&
		(a.datepicker.regional[c] = a.datepicker.regional[b] = d, a.datepicker.regional.en = a.datepicker.regional[""], a.datepicker.setDefaults(d))
	...
	Eb.moment = function () {
		...
		var c = 0;
		return b.isDuration(a) && (c = 24 * Math.floor(a.asDays())), this.hours(c +
		...
		return b.fn.utc.call(this), this.year(a[0]).month(a[1]).date(a[2]).hours(a[3]).minutes(a[4]).seconds(a[5]).milliseconds(a[6]), c &&
		...
		return null !=
		a &&
		(delete this._ambigTime, delete this._ambigZone), b.fn.zone.apply(this, arguments)
	}, M.prototype.local = function () {
		var a = this.toArray(), c = this._ambigZone;
		return delete this._ambigTime, delete this._ambigZone, b.fn.local.apply(this, arguments), c &&
		this.year(a[0]).month(a[1]).date(a[2]).hours(a[3]).minutes(a[4]).seconds(a[5]).milliseconds(a[6]), this
