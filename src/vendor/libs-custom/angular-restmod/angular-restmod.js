...
angular.module('platanus.inflector', [])
  ...
  .provider('inflector', [function () {
    ...
    return {
       ...
       * @description
       *
       * Registers a new locale, see the default english locale implementation for information
      ...
      $get: ['$log', function($log) {
        ...
        return {
           ...
           * @param {boolean} _constant If set to false, first letter is not uppercased, defaults
           *   to false.
           ...
           * @description
           *
           * Transforms a string to snakecase, replaces every space, dash and undercore by the
           *   provided separator.
           *
           * @param {string} _string String to transform
           * @param {string} _sep Separator, defaults to '-'
  ...
  return {
    ...
    $get: ['RMModelFactory', '$log', function(buildModel, $log) {
      ...
      var restmod = {
         ...
         * @description
         *
         * The model factory is used to generate new restmod model types. It's recommended to put
         *   models inside factories, this is usefull later when defining relations and
         *   inheritance, since the angular $injector is used by these features. It's also the
         ...
         *```
         *
         * The `_url` parameter is the resource url the generated model will be bound to, if `null`
         *   is given then the model is *nested* and can only be used in another model context.
         *
         * The model also accepts one or more definition providers as one or more arguments after
         *   the _url parameter, posible definition providers are:
         *
         ...
         * @description
         *
         * The mixin factory is used to pack model behaviors without the overload of generating a
         *   new model. The mixin can then be passed as argument to a call to {@link
         *   restmod#model#model} to extend the model capabilities.
         *
...
RMModule.factory('RMCollectionApi', ['RMUtils', function(Utils) {
  ...
  return {
     ...
     * if a function is given, then it will be called with the request object to allow requet
     *   customization.
     * @return {CollectionApi} self
     */
     ...
     * @memberof CollectionApi#
     *
     * @description Adds an item to the back of the collection. This method does not attempt to
     *   send changes to the server. To create a new item and add it use $create or $build.
     *
    ...
    $add: function(_obj, _idx) {
	    ...
	    /**
	     * @description This statement was commented due to fact that you cannot add multiple
     ...
     *
     * If a function is provided then the index of the first item for which the function returns
     *   true is returned.
     *
...
RMModule.factory('RMCommonApi', ['$http', 'RMFastQ', '$log', function($http, $q, $log) {
   ...
   *
   * @property {promise} $promise The last operation promise (undefined if no promise has been
   *   created yet)
   * @property {array} $pending Pending requests associated to this resource (undefined if no
   *   request has been initiated)
   * @property {object} $$cb Scope call backs (undefined if no callbacks have been defined, private
   *   api)
  ...
  var CommonApi = {
     ...
     * @param {function} _fun Function to be executed in with decorated context, this function is
     *   executed in the callee object context.
     * @return {CommonApi} self
     */
     ...
     *
     * For more information aboout contextual hooks, see the {@link CommonApi#decorate}
     *   documentation.
     *
     * Usage:
     *
     ...
     *           // the event is dispatched using the dispatcher function available when
     *   $saveAndTrack was called. this.$dispatch('trace-stored');
     *         });
     *       });
     *     });
     *   }
     ...
     * @memberof CommonApi#
     *
     * @description Promise chaining method, similar to then but executes same callback in success
     *   or error.
     *
     * Usage:
     *
     ...
     *
     * Calls ´$q.finally´ on the collection's last promise, updates last promise with finally
     *   result.
     *
     * Usage:
     *
     ...
     * * Current request execution status can be queried using the $status property (current
     *   request, not last).
     * * The $status property refers to the current request inside $send `_success` and `_error`
     *   callbacks.
     *
      ...
      return this.$always(function() {
        ...
        return $http(_options).then(wrapPromise(this, function() {
          ...
          if(action && action.canceled) {
            ...
            // IDEA: Consider flushing pending request in case of an error. Also continue ignoring
            // requests until the error flag is reset by user.
...
RMModule.factory('RMExtendedApi', ['$q', 'RMPackerCache', function($q, packerCache) {
  ...
  return {
     ...
     * @description
     *
     * Encode and packs object into a server compatible structure that can be used for PUT/POST
     *   operations.
     *
     * ATTENTION: do not override this method to change the object wrapping strategy,
    ...
    $reset: function() {
      ...
      // TODO: find a way of only ignoring requests that will lead to resolution, maybe using
      // action metadata
...
RMModule.factory('RMListApi', [function() {
  ...
  return {
     ...
     * return an empty list and fill it when the async operation finishes. If you don't need the
     *   async behavior then use `$type.list` directly to generate a new list.
     *
     * @param {function} _filter A filter function that should return the list contents as an
     *   array.
     * @return {ListApi} list
     */
...
RMModule.factory('RMRecordApi', ['RMUtils', function(Utils) {
   ...
   * @description
   *
   * Provides record synchronization and manipulation methods. This is the base API for every
   *   restmod record.
   *
   ...
   */
     ...
     * @memberof RecordApi#
     *
     * @description Called the resource's scope $urlFor method to build the url for the record
     *   using the proper scope.
     *
     * By default the resource partial url is just its `$pk` property. This can be overriden to
     *   provide other routing approaches.
     *
     ...
     *
     * // TODO: create special api to hold scope (so it is not necessary to recreate the whole
     *   object every time.)
     *
    ...
    $decode: function(_raw, _mask) {
      ...
      if(this.$pk === undefined || this.$pk === null) this.$pk = this.$type.inferKey(_raw); // TODO:
                                                                                            // warn
                                                                                            // if
                                                                                            // key
                                                                                            // changes
     ...
     * ```
     * Will first fetch the bike data and after it is loaded the new size will be applied and then
     *   the updated model saved.
     *
     ...
     *
     * A patch is only executed if model is identified and a patch property list is given. It is
     *   posible to change the method used for PATCH operations by setting the `patchMethod`
     *   configuration.
     *
     * If resource is new and it belongs to a collection and it hasnt been revealed, then it will
     *   be revealed.
     *
     * The request's promise can be accessed using the `$asPromise` method.
     *
    ...
    $save: function(_patch) {
      return this.$action(function() {
        ...
        if(url) {
          ...
          if(_patch) {
            request = {
              method: this.$type.getProperty('patchMethod', 'PATCH'), // allow user to override
                                                                      // patch method
              url: url,
     ...
     *
     * If object hasn't been revealed, then this method will change the index where object will be
     *   revealed at.
     *
     ...
     *
     * If instance is bound to a collection and it hasnt been revealed (because it's new and hasn't
     *   been saved), then calling this method without parameters will force the object to be added
     *   to the collection.
     *
     * If this method is called with **_show** set to `false`, then the object wont be revealed by
     *   a save operation.
     *
...
RMModule.factory('RMScopeApi', ['RMUtils', function(Utils) {
  ...
  return {
     ...
     * @memberof ScopeApi#
     *
     * @description Builds a new instance of this model, bound to this instance scope, sets its
     *   primary key.
     *
     ...
     * @memberof ScopeApi#
     *
     * @description Builds a new instance of this model, does not assign a pk to the created
     *   object.
     *
     * ATTENTION: item will not show in collection until `$save` is called. To reveal item before
     *   than call `$reveal`.
     *
     ...
     *
     * ATTENTION: does not automatically reveal item in collection, chain a call to $reveal to do
     *   so.
     *
...
RMModule.factory('RMBuilder', ['$injector', 'inflector', '$log', 'RMUtils', function($injector, inflector, $log, Utils) {
   ...
   * ```
   *
   * With the exception of model configuration variables and properties starting with a special
   *   character (**@** or **~**), each property in the definition object asigns a behavior to the
   *   same named property in a model's record.
   *
   ...
   *
   * The following built in property modifiers are provided (see each mapped-method docs for usage
   *   information):
   *
   ...
   * * `decode` sets how an attribute is decoded after being fetch, maps to {@link
   *   BuilderApi#attrDecoder}
   * * `encode` sets how an attribute is encoded before being sent, maps to {@link
   *   BuilderApi#attrEncoder}
   ...
   * then it is considered to be a default value. (same as calling {@link BuilderApi#define} at a
   *   definition function)
   *
   * ```javascript
   ...
   * ```
   *
   * To add a static method or a collection method, you must specify the method scope: , prefix the
   *   definition key with **^**, to add it to the model collection prototype, prefix it with *****
   *   static/collection methods to the Model, prefix the definition property name with **@**
   ...
   * ## Definition function
   *
   * The definition function gives complete access to the model builder api, every model builder
   *   function described in this page can be called from the definition function by referencing
   *   *this*.
   *
   * ```javascript
...
RMModule.factory('RMBuilderComputed', ['restmod',
  function(restmod) {
     ...
     * * `computed` function will be assigned as getter to Model, maps to {@link
     *   RMBuilderComputedApi#attrAsComputed}
     *
     */
    var EXT = {
...
RMModule.factory('RMBuilderExt', ['$injector', '$parse', 'inflector', '$log', 'restmod', function($injector, $parse, inflector, $log, restmod) {
   ...
   *
   * Adds the following property modifiers:
   * * `serialize` sets the encoder and decoder beaviour for an attribute, maps to {@link
   *   BuilderApi#attrSerializer}
   *
   */
  var EXT = {
    /**
     ...
     *
     * Primary keys are passed to scope's url methods to generate urls. The default primary key is
     *   'id'.
     *
...
RMModule.factory('RMBuilderRelations', ['$injector', 'inflector', '$log', 'RMUtils', 'restmod', 'RMPackerCache', function($injector, inflector, $log, Utils, restmod, packerCache) {
   ...
   *
   * Adds the following property modifiers:
   * * `hasMany` sets a one to many hierarchical relation under the attribute name, maps to {@link
   *   RelationBuilderApi#attrAsCollection}
   * * `hasOne` sets a one to one hierarchical relation under the attribute name, maps to {@link
   *   RelationBuilderApi#attrAsResource}
   * * `belongsTo` sets a one to one reference relation under the attribute name, maps to {@link
   *   RelationBuilderApi#attrAsReference}
   * * `belongsToMany` sets a one to many reference relation under the attribute name, maps to
   *   {@link RelationBuilderApi#attrAsReferenceToMany}
   *
   */
  var EXT = {
    /**
     ...
     *
     * A reference relation expects the host object to provide the primary key of the referenced
     *   object or the referenced object itself (including its key).
     *
     ...
     * ```
     *
     * The object generated by the relation is not scoped to the host object, but to it's base
     *   class instead (not like hasOne), so the type should not be nested.
     *
     * Its also posible to override the **foreign key name**.
     *
     * When a object containing a belongsTo reference is encoded for a server request, only the
     *   primary key value is sent using the same **foreign key name** that was using on decoding.
     *   (`user_id` in the above example).
     *
     * @param {string}  _name Attribute name
     * @param {string|object} _model Other model, supports a model name or a direct reference.
     * @param {string} _key foreign key property name (optional, defaults to _attr + '_id').
     * @param {bool} _prefetch if set to true, $fetch will be automatically called on relation
     *   object load.
     * @return {BuilderApi} self
     */
            ...
            .attrDecoder(_attr + 'Id', function(_value) {
              ...
              if(!this[_attr] || this[_attr].$pk !== _value) {
                if(_value !== null && _value !== false) {
                  loadModel();
                  this[_attr] = packerCache.resolve(_model.$new(_value)); // resolve inmediatelly
                                                                          // if cached
     ...
     *
     * A reference relation expects the host object to provide the primary key of the referenced
     *   objects or the referenced objects themselves (including its key).
     *
     ...
     *
     * The object generated by the relation is not scoped to the host object, but to it's base
     *   class instead (unlike hasMany), so the referenced type should not be nested.
     *
     * When a object containing a belongsToMany reference is encoded for a server request, only the
     *   primary key value is sent for each object.
     *
     * @param {string}  _name Attribute name
     * @param {string|object} _model Other model, supports a model name or a direct reference.
     * @param {string} _keys Server name for the property that holds the referenced keys in
...
RMModule.factory('RMModelFactory', ['$injector', '$log', 'inflector', 'RMUtils', 'RMScopeApi', 'RMCommonApi', 'RMRecordApi', 'RMListApi', 'RMCollectionApi', 'RMExtendedApi', 'RMSerializer', 'RMBuilder',
  ...
  return function(_baseUrl, _baseChain) {
     ...
     *
     * @property {object} $type Reference to the type itself, for compatibility with the {@link
     *   ScopeApi}
     *
    ...
    extend(Model, {
       ...
       * * The primary key value extracted by this method should be comparable using the ==
       *   operator.
       *
       ...
       *
       * This method is called to extract raw data from a record to be sent to server (before
       *   packing)
       */
       ...
       *
       * This method is called on every raw record data property to rename it, by default is not
       *   defined.
       *
       ...
       *
       * This method is called when encoding a record to rename the record properties into the raw
       *   data properties, by default is not defined.
       *
       * Override this method to change the property renaming strategy
       *
       ...
       *
       * By default it uses the `inflector.parameterize` method, in 1.2 this will change and the
       *   default behaviour will be to do nothing.
       *
    ...
    builder = new Builder(angular.extend(serializer.dsl(), {
       ...
       * * primaryKey: The model's primary key, defaults to **id**. Keys must use server naming
       *   convention!
       * * urlPrefix: Url prefix to prepend to resource url, usefull to use in a base mixin when
       *   multiples models have the same prefix.
...
RMModule.factory('RMPackerCache', [function() {
   ...
   * @description
   *
   * The packer cache service enables packing strategies to register raw object data that can be
   *   then used by supporting relations during the decoding process to preload other related
   *   resources.
   *
  ...
  return {
    ...
    feed: function(_name, _rawRecords) {
      packerCache[_name] = _rawRecords; // TODO: maybe append new record to support extended
                                        // scenarios.
    },
     ...
     * @memberof PackerCache#
     *
     * @description Searches for data matching the record's pk, if found data is then fed to the
     *   record using $decode.
     *
    ...
    function decode(_from, _to, _prefix, _mask, _ctx) {
      ...
      for(key in _from) {
        if(_from.hasOwnProperty(key)) {
          ...
          if(value !== undefined) _to[decodedName] = value; // ignore value if filter returns
                                                            // undefined
        }
      }
    }
    ...
    return {
      ...
      dsl: function() {
        ...
        return {
           ...
           *
           * For example, to map the response property `stats.created_at` to model's `created`
           *   property.
           *
           * ```javascript
           ...
           * ```
           *
           * It's also posible to use a wildcard '*' as server name to use the default name decoder
           *   as server name. This is used to force a property to be processed on decode/encode
           *   even if its not present on request/record (respectively), by doing this its posible,
           *   for example, to define a dynamic property that is generated automatically before the
           *   object is send to the server.
           *
           * @param {string} _attr Attribute name
           ...
           *
           * An attribute mask prevents the attribute to be loaded from or sent to the server on
           *   certain operations.
           *
           ...
           * @memberof BuilderApi#
           *
           * @description Makes an attribute volatile, a volatile attribute is deleted from source
           *   after encoding.
           *
           * @param {string} _name Attribute name
           * @param {boolean} _isVolatile defaults to true, if set to false then the attribute is
           *   no longer volatile.
           * @return {BuilderApi} self
           */
...
RMModule.factory('DefaultPacker', ['restmod', 'inflector', 'RMPackerCache', function(restmod, inflector, packerCache) {
   ...
   *
   * This is a simplified version of the wrapping structure recommented by the jsonapi.org
   *   standard,
   * it supports side loaded associated resources (via supporting relations) and metadata
   *   extraction.
   *
   ...
   * ### Json root
   *
   * By default the mixin will use the singular model name as json root for single resource
   *   requests
   * and pluralized name for collection requests. Make sure the model name is correctly set.
   *
   * To override the name used by the mixin set the **jsonRootSingle** and **jsonRootMany**
   *   variables. Or set **jsonRoot** to override both.
   *
   ...
   * use `jsonLinks: '.'`. You can also explicitly select which properties to consider links using
   *   an array of property names. To skip links processing altogether, set it to false.
   *
   * Links are expected to use the pluralized version of the name for the referenced model. For
   ...
   * ```
   *
   * Restmod will expect that the Part model plural name is correctly set parts. Only properties
   *   declared as reference relations (belongsTo and belongsToMany) will be correctly resolved.
   *
   * ### Metadata
   *
   ...
   *
   * Just like links, to change the metadata source property set the jsonMeta property to the
   *   desired name, set it to '.' to capture the entire raw response or set it to false to skip
   *   metadata and set it to an array of properties to be extract selected properties.
   *
  ...
  var Utils = {
    ...
    buildArrayType: function(_forceIframe) {
      ...
      if(PROTO_SETTER && !_forceIframe) {
        ...
        //
        // Very nice array subclassing analysis:
        ...
        //
        // Based on the awesome blog post of Dean Edwards:
        ...
        //
        // Even though MS says that removing iframe from DOM will release it's related structures
        // (http://msdn.microsoft.com/en-us/library/ie/gg622929(v=vs.85).aspx), actually keeping it
        // referenced has proven to be enough to keep the structures alive. (that includes our
        // array type)
