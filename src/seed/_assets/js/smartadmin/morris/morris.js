(function () {
	...
	Morris.Grid = (function (_super) {
		...
		Grid.prototype.drawGrid = function () {
			...
			lastY = this.ymax;
			_results = [];
			for (lineY = _i = firstY, _ref = this.yInterval; firstY <= lastY ? _i <= lastY : _i >=
			lastY; lineY = _i += _ref) {
				...
				if (this.options.grid) {
					_results.push(this.drawGridLine("M" +
						this.left +
						"," +
						y +
						"H" +
		...
		Grid.prototype.drawGoal = function (goal, color) {
			return this.raphael.path("M" +
				this.left +
				"," +
				(this.transY(goal)) +
				"H" +
				this.right).attr('stroke', color).attr('stroke-width', this.options.goalStrokeWidth);
		};
		...
		Grid.prototype.drawEvent = function (event, color) {
			return this.raphael.path("M" +
				...
				this.bottom +
				"V" +
				this.top).attr('stroke', color).attr('stroke-width', this.options.eventStrokeWidth);
		};
	...
	Morris.parseDate = function (date) {
		...
		if (m) {
			...
			if (!q[6]) {
				return new Date(parseInt(q[1], 10), parseInt(q[2], 10) -
					1, parseInt(q[3], 10), parseInt(q[4], 10), parseInt(q[5], 10)).getTime();
			} else {
				offsetmins = 0;
				if (q[6] !== 'Z') {
				...
				return Date.UTC(parseInt(q[1], 10), parseInt(q[2], 10) -
					1, parseInt(q[3], 10), parseInt(q[4], 10), parseInt(q[5], 10) + offsetmins);
			}
		} else if (r) {
			secs = parseFloat(r[6]);
			...
			if (!r[8]) {
				return new Date(parseInt(r[1], 10), parseInt(r[2], 10) -
					1, parseInt(r[3], 10), parseInt(r[4], 10), parseInt(r[5], 10), isecs, msecs).getTime();
			} else {
				offsetmins = 0;
				if (r[8] !== 'Z') {
				...
				return Date.UTC(parseInt(r[1], 10), parseInt(r[2], 10) -
					1, parseInt(r[3], 10), parseInt(r[4], 10), parseInt(r[5], 10) + offsetmins, isecs, msecs);
			}
		} else {
	...
	Morris.Line = (function (_super) {
		...
		Line.prototype.generatePaths = function () {
			...
			return this.paths = (function () {
				...
				_results = [];
				for (i = _i = 0, _ref = this.options.ykeys.length; 0 <= _ref ? _i < _ref : _i >
				_ref; i = 0 <= _ref ? ++_i : --_i) {
		...
		Line.prototype.drawSeries = function () {
			...
			for (i = _i = _ref = this.options.ykeys.length - 1; _ref <= 0 ? _i <= 0 : _i >= 0; i = _ref <=
			0 ? ++_i : --_i) {
				path = this.paths[i];
				if (path !== null) {
			...
			this.seriesPoints = (function () {
				...
				_results = [];
				for (i = _j = 0, _ref1 = this.options.ykeys.length; 0 <= _ref1 ? _j < _ref1 : _j >
			...
			_results = [];
			for (i = _j = _ref1 = this.options.ykeys.length - 1; _ref1 <= 0 ? _j <= 0 : _j >=
			0; i = _ref1 <= 0 ? ++_j : --_j) {
		...
		Line.prototype.hilight = function (index) {
			...
			if (this.prevHilight !== null && this.prevHilight !== index) {
				for (i = _i = 0, _ref = this.seriesPoints.length - 1; 0 <= _ref ? _i <= _ref : _i >=
				_ref; i = 0 <= _ref ? ++_i : --_i) {
			...
			if (index !== null && this.prevHilight !== index) {
				for (i = _j = 0, _ref1 = this.seriesPoints.length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >=
				_ref1; i = 0 <= _ref1 ? ++_j : --_j) {
	...
	Morris.Bar = (function (_super) {
		...
		Bar.prototype.calc = function () {
			...
			this.calcBars();
			if (this.options.hideHover === false) {
				return (_ref = this.hover).update.apply(_ref, this.hoverContentForRow(this.data.length -
					1));
			}
		};
		...
		Bar.prototype.drawXAxis = function () {
			...
			prevLabelMargin = null;
			_results = [];
			for (i = _i = 0, _ref = this.data.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <=
			_ref ? ++_i : --_i) {
		...
		Bar.prototype.drawSeries = function () {
			...
			barWidth = (groupWidth * this.options.barSizeRatio - this.options.barGap * (numBars - 1)) /
				numBars;
	...
	Morris.Donut = (function () {
		...
		Donut.prototype.redraw = function () {
			...
			for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
				...
				seg = new Morris.DonutSegment(cx, cy, w * 2, w, last, next, this.options.colors[idx %
				this.options.colors.length], this.options.backgroundColor, d, this.raphael);
				seg.render();
	...
	Morris.DonutSegment = (function (_super) {
		...
		DonutSegment.prototype.calcArcPoints = function (r) {
			return [this.cx + r * this.sin_p0, this.cy + r * this.cos_p0, this.cx +
			r *
			this.sin_p1, this.cy + r * this.cos_p1];
		};
		...
		DonutSegment.prototype.drawDonutSegment = function (path, fillColor, strokeColor,
			hoverFunction) {
			return this.raphael.path(path).attr({
				fill: fillColor,
				stroke: strokeColor,
