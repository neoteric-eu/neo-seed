...
!function (a) {
	function b(a, b, c, d) {
		...
		if (e || f) {
			...
			for (var j = 0; j < g.length; ++j)g[j].x && e && (g[j].number = !1), g[j].y &&
			f &&
			(g[j].number = !1)
		}
	}
	...
	function e(b, c, e) {
		if ("categories" == b[c].options.mode) {
			if (!b[c].categories) {
				...
				if (a.isArray(h))for (var i = 0; i <
				h.length; ++i)g[h[i]] = i; else for (var j in h)g[j] = h[j];
				b[c].categories = g
			}
	...
	function f(a, b, d) {
		for (var e = a.points, f = a.pointsize, g = a.format, h = b.charAt(0), i = c(d), j = 0; j <
		e.length; j += f)if (null != e[j])for (var k = 0; f > k; ++k) {
			var l = e[j + k];
